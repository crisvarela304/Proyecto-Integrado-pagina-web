...
class Mensaje(models.Model):
    ...
    def clean(self):
        ...
        if conversacion:
            ...
            if autor:
                if not conversacion.puede_acceder(autor):
        ...
        if not receptor and conversacion and autor:
            self.receptor = conversacion.get_otro_participante(autor)
            # Actualizar variable local para la siguiente validación
            try:
                receptor = self.receptor
            except Exception:
                pass

        if receptor and conversacion:
            if not conversacion.puede_acceder(receptor):
